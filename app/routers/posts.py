...
def get_posts(db: Session = Depends(get_db), current_user: int = Depends(oauth.get_current_user), limit: int = 10, search: Optional[str] = ""):
    # ***** normal squal query *****
    ...
    results = db.query(models.Post, func.count(models.Vote.post_id).lable("votes")).join(
...
def update_post(id: int, post: schemas.CreatePost, db: Session = Depends(get_db), current_user: int = Depends(oauth.get_current_user)):
    # cursor.execute(
